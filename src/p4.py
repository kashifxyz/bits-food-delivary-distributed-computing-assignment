...
Responsibilities:
- Receive an ORDER message from P2 (Restaurant B).
...
- Participate in the Chandy-Lamport snapshot algorithm: on receiving a
  ...
  message (P4 is a leaf process, so there is nothing to forward to).
"""
...
def handle_order(msg: dict) -> None:
    """Process an incoming order from P2 and run it through the delivery lifecycle."""
    order_data = msg["data"]
...
def handle_marker(msg: dict) -> None:
    ...
    # P4 has no downstream process to forward the marker to (leaf process),
...
def start_server() -> None:
    ...
    server.listen(5)
    print(f"{PROCESS_NAME} (Delivery Partner 2 - Fleet Runner B) listening on port {PORTS[PROCESS_NAME]}...")
